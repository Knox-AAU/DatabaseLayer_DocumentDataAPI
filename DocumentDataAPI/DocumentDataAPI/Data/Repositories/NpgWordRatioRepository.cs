...
public class NpgWordRatioRepository : IWordRatioRepository
{
    ...
    public WordRatioModel? Get(long id)
    {
        ...
        return con.QuerySingleOrDefault<WordRatioModel>("select * from word_ratios where id=@Id", new { id });
    }
    ...
    public int Add(WordRatioModel entity)
    {
        ...
        using IDbConnection con = _connectionFactory.CreateConnection();
        return con.Execute("insert into word_ratios(documents_id, word, amount, percent, rank)" +
    ...
    public int Update(WordRatioModel entity)
    {
        ...
        _logger.LogTrace("WordRatio: {WordRatio}", entity);
        using IDbConnection con = _connectionFactory.CreateConnection();
        return con.Execute("update word_ratios set word = @Word, amount = @Amount, percent = @Percent, rank = @Rank " +
