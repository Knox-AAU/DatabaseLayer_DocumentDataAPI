...
public class NpgDataSourceRepository : IDataSourceRepository
{
    ...
    public async Task<DataSourceModel?> Get(long id)
    {
        ...
        return await con.QueryFirstOrDefaultAsync<DataSourceModel>("select * from data_sources where id=@Id", new { id });
    }
    ...
    public async Task<long> GetCountFromId(long id)
    {
        ...
        return await con.QuerySingleOrDefaultAsync<int>(
            ...
            "where data_sources_id=@Id", new { id });
    }
    ...
    public async Task<IEnumerable<DataSourceModel>> GetByName(string name)
    {
        ...
        return await connection.QueryAsync<DataSourceModel>("select * from data_sources where name = @Name", new { name });
    }
}
